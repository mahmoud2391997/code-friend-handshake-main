...
const SaleDetailModal: React.FC<SaleDetailModalProps> = ({ sale, onClose, onSave, branches, products, inventory, customers }) => {
    ...
    useEffect(() => {
        ...
        setEditableSale(isCreating ? {
            brand: defaultBrand,
            branchId: Number(brandBranches[0]?.id) || 0,
    ...
    const handleFieldChange = (field: keyof Sale, value: string | number) => {
        if (field === 'brand') {
            ...
            setEditableSale(prev => ({ ...prev, brand: value as 'Arabiva' | 'Generic', branchId: Number(brandBranches[0]?.id) || 0 }));
        } else {
    ...
    const handleCustomerChange = (customerId: number) => {
        ...
        if (selectedCustomer) {
            setEditableSale(prev => ({ 
                ...prev, 
                customerId: typeof selectedCustomer.id === 'number' ? selectedCustomer.id : parseInt(String(selectedCustomer.id)),
