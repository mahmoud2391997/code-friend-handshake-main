            ...
            .then(({ data }) => {
                if (data) {
                    ...
                    if (user?.branchId) {
                        ...
                        const branchInventory = inventory.filter(i => String(i.branchId) === String(user.branchId));
                        const inventoryItem = branchInventory.find(i => String(i.id) === String(localProduct.id));
                        stock = inventoryItem ? inventoryItem.currentStock : 0;
                    } else {
                        // Non-branch users: sum inventory across all branches
                        const inventoryItems = inventory.filter(i => String(i.id) === String(localProduct.id));
                        stock = inventoryItems.reduce((total, item) => total + item.currentStock, 0);
                    }
