...
import { createVoucher, deleteVoucher, fetchVouchers, updateVoucher } from '../src/store/slices/vouchersSlice';
        ...
        <div className="inventory-vouchers-container-enhanced">
            ...
            <div className="inventory-vouchers-actions">
                <button 
                    ...
                    onClick={() => { setModalMode('create'); setIsVoucherModalOpen(true); }}
                >
                        ...
                        <div key={String(voucher._id || voucher.id)} className="voucher-card-enhanced">
                            ...
                            <div className="voucher-card-actions">
                                <button 
                                    ...
                                    title="عرض التفاصيل"
                                    onClick={() => { setSelectedVoucher(voucher); setModalMode('view'); setIsVoucherModalOpen(true); }}
                                >
                                    👁️ عرض
                                </button>
                                ...
                                <button 
                                    className="btn btn-ghost btn-sm voucher-action-btn"
                                    title="تعديل الإذن"
                                    onClick={() => { setSelectedVoucher(voucher); setModalMode('edit'); setIsVoucherModalOpen(true); }}
                                >
                                    ✏️ تعديل
                                </button>
            ...
            {/* VoucherModal */}
            {isVoucherModalOpen && modalMode === 'create' && (
                <VoucherModal 
                    ...
                    onSave={(voucher) => {
                        dispatch(createVoucher(voucher))
                          .unwrap()
                          .then(() => { 
                              addToast('تم إنشاء السند بنجاح', 'success'); 
                              setIsVoucherModalOpen(false);
                              dispatch(fetchVouchers()); 
                          })
                          .catch(() => addToast('فشل في إنشاء السند', 'error'));
                    }}
                />
            )}
            {isVoucherModalOpen && modalMode === 'view' && selectedVoucher && (
                <VoucherModal 
            ...
            {isVoucherModalOpen && modalMode === 'edit' && selectedVoucher && (
                <VoucherModal 
                    onClose={() => { setIsVoucherModalOpen(false); setSelectedVoucher(null); }}
                    onSave={(voucher) => {
                        const id = String(selectedVoucher._id || selectedVoucher.id);
                        dispatch(updateVoucher({ id, data: voucher }))
                          .unwrap()
                          .then(() => { 
                              addToast('تم تحديث السند بنجاح', 'success'); 
                              setIsVoucherModalOpen(false);
                              setSelectedVoucher(null);
                              dispatch(fetchVouchers()); 
                          })
                          .catch(() => addToast('فشل في تحديث السند', 'error'));
                    }}
                    initialVoucher={selectedVoucher}
                />
            )}
