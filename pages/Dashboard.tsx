...
const Dashboard: React.FC<DashboardProps> = ({ sales, purchases, employees, inventory, products, branches, settings, accounts, expenses, renewables, leaveRequests, advanceRequests, generalRequests, suppliers, setActiveView }) => {
        ...
        <div className="dashboard-container">
            {lowStockItems.length > 0 && (
                <div className="glass-pane" style={{ padding: '1.5rem', borderLeft: '5px solid #f59e0b' }}>
                    ...
                    <p style={{color: 'var(--text-secondary)'}}>
                        بعض المنتجات وصلت إلى الحد الأدنى للمخزون أو أقل. قم بزيارة صفحة <a href="#" onClick={(e) => { e.preventDefault(); setActiveView && setActiveView('Inventory/Products'); }} style={{color: 'var(--primary-color)', fontWeight: 600}}>المخزون</a> للمزيد من التفاصيل.
                    </p>
                </div>
            )}
            {expiringSoonItems.length > 0 && (
                <div className="glass-pane" style={{ padding: '1.5rem', borderLeft: '5px solid #f59e0b' }}>
                    ...
                    <p style={{color: 'var(--text-secondary)'}}>
                        {expiringSoonItems.length} منتجات ستنتهي صلاحيتها خلال 30 يومًا. قم بزيارة صفحة <a href="#" onClick={(e) => { e.preventDefault(); setActiveView && setActiveView('Inventory/Products'); }} style={{color: 'var(--primary-color)', fontWeight: 600}}>المخزون</a> للمزيد من التفاصيل.
                    </p>
                </div>
            )}
            ...
            <div className="dashboard-main-grid-alt">
                ...
                <div className="dashboard-charts-column">
                    ...
                    <div className="dashboard-charts-grid">
                        ...
                        {/* Quick Actions */}
                        <div className="glass-pane" style={{ padding: '1.5rem' }}>
                            <h3 style={{ fontSize: '1.25rem', fontWeight: 600, marginBottom: '1rem' }}>إجراءات سريعة</h3>
                            <div className="quick-actions-list" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0.75rem' }}>
                                <button className="quick-action-link" onClick={() => setActiveView && setActiveView('Sales/Invoices')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <ShoppingCartIcon className="icon" /><span>فاتورة بيع جديدة</span>
                                </button>
                                <button className="quick-action-link" onClick={() => setActiveView && setActiveView('Purchases/Invoices')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <TruckIcon className="icon" /><span>فاتورة شراء جديدة</span>
                                </button>
                                <button className="quick-action-link" onClick={() => setActiveView && setActiveView('Inventory/Products')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <CubeIcon className="icon" /><span>إدارة المنتجات</span>
                                </button>
                                <button className="quick-action-link" onClick={() => setActiveView && setActiveView('HR/Employees')} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                    <UsersIcon className="icon" /><span>إضافة موظف جديد</span>
                                </button>
                            </div>
                        </div>
                    </div>
