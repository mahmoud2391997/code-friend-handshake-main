...
import InventoryExcelImport from '../components/InventoryExcelImport';
...
const Inventory: React.FC<InventoryProps> = ({ inventory, products, branches, onUpdateItem, onTransferInventory, onAdjustInventory, movements = [] }) => {
    ...
    const handleMinStockChange = (item: InventoryItem, newMinStock: number) => {
        onUpdateItem({ ...item, minimumStock: newMinStock });
    ...
    const inventoryWithProductInfo = inventory.map(i => ({
        ...i,
        // Ensure currentStock is available, fallback to quantity or 0
        currentStock: i.currentStock ?? i.quantity ?? 0,
    ...
    const stats = useMemo(() => {
        ...
        const activeItems = filteredInventory.filter(i => i.currentStock > 0).length;
        const lowStockItems = filteredInventory.filter(i => i.currentStock <= i.minimumStock && i.minimumStock > 0).length;
        ...
        const totalValue = filteredInventory.reduce((sum, i) => {
            ...
            return sum + unitCost * i.currentStock;
        }, 0);
        const unavailableItems = filteredInventory.filter(i => i.currentStock <= 0).length;
        const lockedItems = 0;
            ...
            <div className="glass-pane inventory-page-container">
                        ...
                        <div className="inventory-filters">
                            <div className="inventory-filter-group" style={{ gap: '0.5rem' }}>
                                ...
                                <button 
                                    ...
                                    onClick={async () => {
                                        try {
                                            const response = await fetch('http://localhost:4100/api/inventory/seed', {
                                                method: 'POST',
                                                headers: { 'Content-Type': 'application/json' }
                                            });
                                            if (response.ok) {
                                                addToast('تم تحميل بيانات المخزون بنجاح!', 'success');
                                                window.location.reload();
                                            } else {
                                                addToast('فشل في تحميل بيانات المخزون', 'error');
                                            }
                                        } catch (error) {
                ...
                <div className="inventory-table-wrapper">
                    ...
                    <table className="inventory-table">
                        ...
                        <tbody>
                            {(activeTab === 'low' ? filteredInventory.filter(i => i.currentStock <= i.minimumStock && i.minimumStock > 0) : filteredInventory).map(item => {
                                const isLowStock = item.currentStock <= item.minimumStock && item.minimumStock > 0;
                                return (
                                    <tr key={`${item.branchId}-${item.id}`} className={isLowStock ? 'low-stock-row' : ''}>
                                        <td>
                                        ...
                                        <td style={{ fontWeight: 'bold', fontSize: '1.1rem', color: isLowStock ? '#f87171' : 'var(--text-primary)' }}>
                                            {item.product?.baseUnit === 'pcs' ? item.currentStock.toLocaleString() : item.currentStock.toFixed(2)}
                                        </td>
                                        <td>
