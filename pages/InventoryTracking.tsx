...
const Inventory: React.FC = () => {
    ...
    const [showSupplyChainImport, setShowSupplyChainImport] = useState(false);
    ...
    useEffect(() => {
        ...
        import('../src/store/slices/supplyChainSlice').then(({ fetchSupplyChainItems }) => {
    ...
    return (
        <>
                    ...
                    <div className="inventory-page-actions">
                        <div className="inventory-filters">
                            <div className="inventory-filter-group" style={{ gap: '0.5rem' }}>
                                ...
                                <button className="btn btn-ghost" onClick={() => setShowSupplyChainImport(true)}>استيراد من سلسلة التوريد</button>
                            </div>
            ...
            {showSupplyChainImport && (
                <div className="modal-backdrop" onClick={() => setShowSupplyChainImport(false)}>
                    <div className="modal-content glass-pane" style={{maxWidth: '800px'}} onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <h2 style={{fontSize: '1.25rem', fontWeight: 600}}>استيراد من سلسلة التوريد</h2>
                        </div>
                                ...
                                <tbody>
                                    {(supplyChainItems || []).map(item => (
                                        <tr key={item.id}>
                                            ...
                                            <td>{item.quantity}</td>
                                            <td>{item.unit || 'pcs'}</td>
                                            <td>{item.currentStatus || 'Stored'}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        <div className="modal-footer" style={{justifyContent: 'space-between'}}>
                            <button onClick={() => setShowSupplyChainImport(false)} className="btn btn-ghost">إلغاء</button>
                            <button
                                className="btn btn-primary"
                                onClick={() => {
                                    // Handle import logic here
                                    setShowSupplyChainImport(false);
                                    addToast('تم الاستيراد بنجاح', 'success');
                                }}
                            >استيراد المحدد</button>
                        </div>
                    </div>
                </div>
            )}
